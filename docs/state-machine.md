...
Quick, direct Hotfix, and Tweak are exempt from execution-plan and review-receipt requirements on their normal bounded path; each closes with `test_result: pass`. If Quick, direct Hotfix, Tweak, or lightweight enters DP-5 debugging, a valid workflow receipt matching the current selection authorizes planless attempts and binds the ledger to its stable authorization identity. Full/legacy Hotfix continues to require a current execution plan.
...
4. Before recording an attempt, require a current execution plan for Full/legacy Hotfix; for Quick/direct Hotfix/Tweak/lightweight, require a valid workflow receipt matching the current selection and bind the ledger to its stable authorization identity. Then record one distinct evidence-backed attempt with `ssf debug attempt record`; Wave Review repair failures remain separate

...
- **证据门禁**：每次失败修复必须通过 `ssf debug attempt record` 写入结构化 ledger，包含唯一 attempt id、摘要和 change 目录内的物理证据文件。Full/legacy Hotfix 必须有 current、有效的 execution plan；Quick、direct Hotfix、Tweak 和 lightweight 可凭有效且匹配当前选择的 workflow receipt 无计划记录，ledger 绑定到 receipt 的稳定 authorization identity。缺失、陈旧或不匹配的执行上下文会被拒绝。Wave Review 的 repair failure 不计入 debugging attempt；重复证据也会被拒绝。

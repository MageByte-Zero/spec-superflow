...
- **证据门禁**：每次失败修复必须通过 `ssf debug attempt record` 写入结构化 ledger，包含唯一 attempt id、摘要和 change 目录内的物理证据文件。Wave Review 的 repair failure 不计入 debugging attempt；重复证据和陈旧 execution context 会被拒绝。

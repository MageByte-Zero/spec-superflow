...
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_1_result "confirmed: <one-line summary>"
node scripts/spec-superflow.mjs state set <change-dir> dp_1_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```

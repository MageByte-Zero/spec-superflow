...
Converts planning artifacts into a single execution handshake: `execution-contract.md`. Load the baseline with `node scripts/spec-superflow.mjs runtime asset read templates/execution-contract.md`.

Read before generating: `.spec-superflow.yaml` (especially `dp_0_decisions`),
`proposal.md`, `specs/`, `design.md`, `tasks.md`, then load
`docs/artifact-contract.md` with `node scripts/spec-superflow.mjs runtime asset read docs/artifact-contract.md`.
...
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_3_result "approved: <summary>"
node scripts/spec-superflow.mjs state set <change-dir> dp_3_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
For a legacy Hotfix, after writing the minimal contract, run `node scripts/spec-superflow.mjs state init <change-dir>` or `node scripts/spec-superflow.mjs state rebuild <change-dir>` so `contract_hash` is recorded. DP-3 remains mandatory before build.

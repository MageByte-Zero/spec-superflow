...
Do not transition to `executing` until `execution show` reports `current: true` and the phase guard passes. A revised plan must use `npx --yes --package spec-superflow@0.9.1 ssf execution revise`; it creates a new revision and invalidates receipts from the prior revision.
...
Only after an explicit user override recorded by `npx --yes --package spec-superflow@0.9.1 ssf execution plan --override`. Executes in the current session and still writes one review receipt per planned wave.

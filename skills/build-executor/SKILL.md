...
1. Run the isolation check:
   ```bash
   node scripts/spec-superflow.mjs isolate <change-dir>
   ```
...
2. If `node scripts/spec-superflow.mjs isolate` exits non-zero: STOP. Do not edit `main`/`master` in place.
   Ask the user for explicit approval (and re-run with `node scripts/spec-superflow.mjs isolate <change-dir> --force`
...
```bash
node scripts/spec-superflow.mjs execution recommend <change-dir> \
...
node scripts/spec-superflow.mjs execution plan <change-dir> \
...
node scripts/spec-superflow.mjs execution show <change-dir> --json
```
...
Do not transition to `executing` until `execution show` reports `current: true` and the phase guard passes. A revised plan must repeat `node scripts/spec-superflow.mjs execution recommend` and use `node scripts/spec-superflow.mjs execution revise --confirm`; it creates a new revision and invalidates receipts from the prior revision.
...
4. After every wave, write a non-empty persisted regular-file review report (separate from the implementer's report), then record exactly one receipt that names that review report:
   ```bash
   node scripts/spec-superflow.mjs execution review <change-dir> \
...
### Per-Task Loop
1. **Dispatch implementer**: Load the template with `node scripts/spec-superflow.mjs runtime asset read skills/build-executor/implementer-prompt.md`. Extract task brief with `scripts/task-brief PLAN_FILE N`. Include: where task fits, brief path, interfaces from prior tasks, report file path.
...
3. **Review**: Load `node scripts/spec-superflow.mjs runtime asset read skills/build-executor/task-reviewer-prompt.md`. Reviewer returns spec compliance + code quality verdicts with the wave ID, git range, report path, and `pass`/`fail` receipt command.
...
```bash
node scripts/spec-superflow.mjs runtime config --resolve-model <profile>
```
...
### Progress Ledger
Track in `.superpowers/sdd/progress.md`. Check for existing ledger — completed tasks are done. After each batch: `node scripts/spec-superflow.mjs state set <change-dir> batches_completed <N>`.

## Inline Execution Mode

Only after a user-confirmed `inline` selection is recorded by `node scripts/spec-superflow.mjs execution plan --confirm`; a non-recommended selection also records `--acknowledge-recommendation`. Executes in the current session and still writes one review receipt per planned wave.
...
```bash
node scripts/spec-superflow.mjs checkpoint save <change-dir> \
...
when `node scripts/spec-superflow.mjs checkpoint list` reports it as stale.
...
DP-4 is written by `node scripts/spec-superflow.mjs execution plan`; do not write it with raw `state set`.
DP-5 (debug escalation): `node scripts/spec-superflow.mjs state set <change-dir> dp_5_result "<resolution>"` + timestamp.

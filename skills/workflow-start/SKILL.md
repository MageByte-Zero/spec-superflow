...
`exploring` → `specifying` → `bridging` → `approved-for-build` → `executing` → `closing`, with `debugging` side-path from `executing`, and `abandoned` as terminal. If a transition is ambiguous, run `node scripts/spec-superflow.mjs runtime asset read docs/state-machine.md`.
...
1. **Update check**: Run `node scripts/spec-superflow.mjs runtime check-update`. Exit 0 → continue. Exit 1 → non-blocking upgrade reminder. Exit 2 → skip.
...
3. **Overlay recovery scan**: Run `node scripts/spec-superflow.mjs handoff list <change-dir> --json` and `node scripts/spec-superflow.mjs checkpoint list <change-dir> --json`. A `result-ready` handoff requires explicit review and `node scripts/spec-superflow.mjs handoff resolve` before resuming the affected work. An `active` handoff is non-blocking side work. Show a non-stale checkpoint as recovery context; show a stale checkpoint only as historical evidence.

## Execution-Control Recovery Scan

4. **Execution-control recovery scan**: For Full or legacy Hotfix in `approved-for-build`, `executing`, or `debugging`, run `node scripts/spec-superflow.mjs execution show <change-dir> --json`. Treat only `current: true` plus `waves[].eligible: true` as permission to start a wave. Do not require this scan for Quick, Tweak, or a valid direct Hotfix receipt.
...
node scripts/spec-superflow.mjs workflow recommend <change-dir> --task-count <n> --file-count <n> --config-doc-only no --schema-api-change no --new-module no --behavioral-constraint-change <yes|no> --cross-module-change <yes|no> --uncertainty low --request-kind <standard|incident>
node scripts/spec-superflow.mjs workflow accept <change-dir> --source direct-request --verification <tdd|new-test|bounded>
```
...
2. If the state file is absent or `dp_0_confirmed` is `false`/null, run `node scripts/spec-superflow.mjs state init <change-dir>` before `show`; initialization must leave DP-0 unconfirmed.
...
4. For `auto`/`null`/unset, run `node scripts/spec-superflow.mjs workflow show <change-dir> --json` before collecting or changing any facts. A missing receipt is represented as `needs-input` with all eight fixed facts in `missing_facts`.
...
6. Run `node scripts/spec-superflow.mjs workflow recommend <change-dir> ...` once with one complete fact snapshot.
...
8. A recommended low-risk Quick or incident Hotfix is accepted only with
   ...
   `node scripts/spec-superflow.mjs workflow select <change-dir> --mode <full|hotfix|tweak> --confirm --reason "<user choice>"`. For a risk-signalled Quick choice, run `workflow select --mode quick --confirm --acknowledge-recommendation --verification <tdd|new-test|bounded>`.
...
11. Keep `node scripts/spec-superflow.mjs runtime infer <change-dir>` only for legacy artifact inference and validation compatibility; it cannot replace user selection at intake.
...
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_0_decisions "<combined summary preserving scope, artifact_language, and workflow_path>"
node scripts/spec-superflow.mjs state set <change-dir> dp_0_result confirmed
...
Guard: `node scripts/spec-superflow.mjs runtime guard check <dir> exploring specifying --json` → fail = BLOCK. User knows what they want, artifacts missing/incomplete.
...
The current planned wave is implemented and ready for spec-compliance + code-quality verification. A reviewer must write an `node scripts/spec-superflow.mjs execution review <change-dir> --wave <id> --base <sha> --head <sha> --report <path> --verdict <pass|fail>` receipt before any dependent wave or closing transition. Quick, Tweak, and direct Hotfix use their verification summary instead.
...
```bash
node scripts/spec-superflow.mjs handoff create <change-dir> \
  --type prototype --objective "<confirmed objective>" \
  --expected-output "<expected evidence>" --acceptance "<completion criterion>"
node scripts/spec-superflow.mjs isolate <change-dir> prototype-<handoff-id>
```
...
Post-transition: 💡 `node scripts/spec-superflow.mjs inject <change-dir>` to update phase-guard artifacts.
...
- No implementation for Full or legacy Hotfix without a current `node scripts/spec-superflow.mjs execution plan`; no state transition based on an unverified DP-4 string

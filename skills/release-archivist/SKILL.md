...
- Run `node scripts/spec-superflow.mjs audit <change-dir>` — include `decision-point-audit.md` in archive

### DP-6 (Verification Outcome, Full/legacy Hotfix)
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_6_result "<pass|conditional|fail>: <summary>"
node scripts/spec-superflow.mjs state set <change-dir> dp_6_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
```bash
node scripts/spec-superflow.mjs state set <change-dir> test_result pass
```

### DP-7 (Archive Confirmation, Full/legacy Hotfix)
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_7_result "confirmed: <archive summary>"
node scripts/spec-superflow.mjs state set <change-dir> dp_7_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
run `node scripts/spec-superflow.mjs state transition <change-dir> closing`.
...
- **Missing files**: If audit can't generate, run `node scripts/spec-superflow.mjs audit` manually

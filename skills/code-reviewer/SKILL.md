...
4. Require the reviewer to write a non-empty persisted review report at `[REVIEW_REPORT_FILE]`, then record that exact path in the wave receipt: `node scripts/spec-superflow.mjs execution review <change-dir> --wave <id> --base <sha> --head <sha> --report <review-report-path> --verdict <pass|fail>`.
...
| Proceeding without a wave receipt | Record `pass`/`fail` via `node scripts/spec-superflow.mjs execution review` before the next dependent wave |

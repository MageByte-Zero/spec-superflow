...
Run: `node scripts/spec-superflow.mjs runtime config --get artifacts.order` — generate in configured order (default: proposal → specs → design → tasks). Run with `artifacts.skip` — skip any listed artifacts.
...
```bash
node scripts/spec-superflow.mjs state set <change-dir> dp_2_result "approved: <summary>"
node scripts/spec-superflow.mjs state set <change-dir> dp_2_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```

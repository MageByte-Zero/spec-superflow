...
先检查 `$ARGUMENTS` 是否为非空目标：为空时运行 `node scripts/spec-superflow.mjs resume --json`，让 CLI 按唯一活跃 change 的既有确定性规则选择；非空时运行 `node scripts/spec-superflow.mjs resume --json "$ARGUMENTS"`，将整个目标作为单一字面参数。

...
- [ ] 1.1 为 ADDED、MODIFIED、REMOVED、RENAMED 的基线应用编写失败测试。
- [ ] 1.2 运行 `node --test tests/lib/cmd-sync-paths.test.mjs` 并确认失败。
- [ ] 1.3 在 `scripts/lib/cmd-sync.mjs` 实现最小 delta 应用与确定性哈希。
- [ ] 1.4 运行同步测试并确认通过。
- [ ] 1.5 审查基线不包含 delta 标头。

## 2. Batch 2: Receipt-gated closure

- [ ] 2.1 为缺失或过期回执编写 closing guard 失败测试。
- [ ] 2.2 运行 `node --test tests/lib/guard-specs-merged.test.mjs` 并确认失败。
- [ ] 2.3 在状态加载器和 `specs-merged` guard 中实现回执持久化与验证。
- [ ] 2.4 运行 guard 测试并确认通过。
- [ ] 2.5 运行完整测试套件并确认通过。

...
describe('ssf execution', () => {
  ...
  it('requires confirmation and acknowledgement when revising away from the recommendation', () => {
    const initial = runSsf(['execution', 'plan', changeDir, '--mode', 'sdd',
    ...
    const rejected = runSsf(['execution', 'revise', changeDir, '--mode', 'sdd', '--confirm',
  ...
  it('requires acknowledgement when a revision differs from the recommendation', () => {
    ...
    assert.match(revised.stderr, /acknowledge/i);
  });

  it('switches SDD revisions to Native final review by default', () => {
    const initial = runSsf(['execution', 'plan', changeDir, '--mode', 'sdd',
      '--reason', 'parallel work', '--wave', 'wave-1:parallel:1.1,1.2']);
    assert.equal(initial.exitCode, 0, initial.stderr);

    const revised = runSsf(['execution', 'revise', changeDir, '--mode', 'inline',
      '--reason', 'continue locally', '--wave', 'wave-1:serial:1.1', '--json']);
    assert.equal(revised.exitCode, 0, revised.stderr);
    assert.equal(revised.json.plan.review_policy, 'final');
  });
